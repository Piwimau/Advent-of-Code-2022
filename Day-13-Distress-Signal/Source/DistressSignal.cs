...
namespace DistressSignal.Source;
...
internal sealed class DistressSignal {
    ...
    private sealed class ListPacket : IPacket {
        ...
        public static ListPacket Parse(ReadOnlySpan<char> s) {
            ...
            for (int i = 1; i < (s.Length - 1); i++) {
                ...
                if (c == '[') {
                    ...
                    current.AddSubPacket(new ValuePacket(int.Parse(value)));
                }
                else if (c != ',') {
                    throw new ArgumentOutOfRangeException(
                        nameof(s),
        ...
        public void AddSubPacket(IPacket subPacket) {
            ArgumentNullException.ThrowIfNull(subPacket, nameof(subPacket));
            if (subPacket == this) {
                throw new ArgumentOutOfRangeException(
                    nameof(subPacket),
    ...
    internal static void Solve(TextWriter textWriter) {
        ...
        ImmutableArray<ListPacket> packets = [.. File.ReadLines(InputFile)
