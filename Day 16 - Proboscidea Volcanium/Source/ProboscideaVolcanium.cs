...
internal sealed partial class ProboscideaVolcanium {
    ...
    private sealed class Valve {
        ...
        public Valve(bool isSource, int flowRate, uint mask) {
            ArgumentOutOfRangeException.ThrowIfNegative(flowRate, nameof(flowRate));
            if (isSource) {
                if (flowRate != 0) {
    ...
    private static FrozenDictionary<Valve, ImmutableArray<Valve>> ParseValves(
        ...
        foreach (string line in lines) {
            ...
            int flowRate = int.Parse(match.Groups["flowRate"].ValueSpan);
    ...
    private static int MaxReleasedPressure(
        ...
        ArgumentNullException.ThrowIfNull(destinations, nameof(destinations));
        ArgumentNullException.ThrowIfNull(visitedStates, nameof(visitedStates));
    ...
    internal static void Solve(TextWriter textWriter) {
        ...
        FrozenDictionary<Valve, ImmutableArray<Destination>> destinations =
            Destinations(ParseValves([.. File.ReadLines(InputFile)]));
